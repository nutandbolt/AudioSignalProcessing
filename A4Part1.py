...
def extractMainLobe(window, M):
    ...
    mX1=np.zeros(len(mX))
    hN3=int((N+1)/2) # N=16, 8
    hN4=int(N/2)    #N=16, 8
    mX1[:hN3]=mX[hN4:]
    mX1[hN3:]=mX[:hN4]
    plt.plot(mX1)

    moving_index=np.argmax(mX1)
    
    check = True
    i=1
    
    while (check == True):
        if (mX1[moving_index+i]> mX1[moving_index]) :
            check=False
        moving_index+=1
        i+=1   
         
    mX2=np.zeros(2*len(mX1[np.argmax(mX1):moving_index])-1)
    #print(len(mX1[np.argmax(mX1):moving_index]))
    delta=moving_index-np.argmax(mX1)
    mX2[:np.argmax(mX1)]=mX1[np.argmax(mX1)-delta+1:np.argmax(mX1)+delta]
    print(len(mX2))
